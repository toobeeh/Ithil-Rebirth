...
class PalantirDatabase {
    ...
    async clearVolatile() {
        ...
        try {
            ...
            await this.get("DELETE FROM Lobbies WHERE json_extract(Lobby, '$.ID') NOT IN (SELECT DISTINCT json_extract(Status, '$.LobbyID') FROM Status WHERE json_extract(Status, '$.LobbyID') IS NOT NULL) AND FROM_UNIXTIME(LobbyID) < DATE_ADD(CURRENT_TIMESTAMP, INTERVAL -60 SECOND)", []);
